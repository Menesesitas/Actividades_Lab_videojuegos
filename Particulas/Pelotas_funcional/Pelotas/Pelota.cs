...
namespace Pelotas
{
    public class Pelota
    {
        ...
        // Tiempo de vida
        private float tiempoVida; // Variable para rastrear el tiempo de vida de la pelota
        private float tiempoMaximoVida = 5f; // Tiempo máximo de vida de la pelota (5 segundos)
        ...
        public Pelota(Random rand,Size size, int index)
        {
            ...
            tiempoVida = 0f; // Inicializar tiempo de vida en cero
        ...
        {
            tiempoVida += deltaTime; // Incrementar tiempo de vida

            // Verificar si la pelota ha existido durante más tiempo del permitido
            if (tiempoVida > tiempoMaximoVida)
            {
                // Generar una nueva pelota y eliminar la actual de la lista de pelotas
                balls.Remove(this);
                Pelota nuevaPelota = new Pelota(rand, space, balls.Count);
                balls.Add(nuevaPelota);
                return; // Salir del método para evitar actualizar la posición de la pelota eliminada
            }
